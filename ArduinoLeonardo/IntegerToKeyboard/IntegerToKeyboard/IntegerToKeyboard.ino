...
// Initialize SoftwareSerial on the specified pins
SoftwareSerial bluetoothSerial(HC06_RX, HC06_TX);
...
bool useDebugPrint=true;  // PRINT TAKE TIME AND DELAY DISABLE IT IF YOU DON T NEED IT  (Switchable with INTEGER)
bool pushPingToHC06=false; // PUSH A MESSAGE EVERY N SECONDS TO NOTIFY ARDUINO IS STILL ALIVE
...
void integerToActions(int integer){
    ...
    switch(integer){
      ...
      case 1533: Mouse.move(0,127); break;
      case 2534: Mouse.move(0,-127); break;
      case 2550:for(int i=0; i<20;i++) Mouse.move(-127,+127); break; // RIGHT TOP  127 is maximum of bit to move apparently
      case 2551:for(int i=0; i<20;i++) Mouse.move(-127,-127); break; // RIGHT DOWN 127 is maximum of bit to move apparently
      case 2552:for(int i=0; i<20;i++) Mouse.move(+127,-127); break; // LEFT DOWN 127 is maximum of bit to move apparently
      case 2553:for(int i=0; i<20;i++) Mouse.move(+127,+127); break; // LEFT TOP 127 is maximum of bit to move apparently
      ...
      case 1633: Mouse.move(0,127); break;
      case 2634: Mouse.move(0,-127); break;
      ...
      case 1409: Keyboard.end(); Keyboard.begin(KeyboardLayout_hu_HU); Serial.println("KeyboardLayout_hu_HU") ; break;
        ...
        case 998: useDebugPrint=true;
        case 999: useDebugPrint=false;
        case 996: pushPingToHC06=true;
        case 997: pushPingToHC06=false;
...
int8_t zero=0;
//0000KKKK
void PushIntegerToUART(int32_t value){
    uint8_t  bytes[4];
    intToBytes(value, bytes);
    for(int i=0; i<4;i++)
      bluetoothSerial.write(zero);
    for(int i=0; i<4;i++)
      bluetoothSerial.write(bytes[i]);
...
void setup() {
  // Initialize Serial Monitor at 9600 bps for debugging
  Serial.begin(9600);

  // Initialize Bluetooth Serial at 9600 bps (default for HC-06)
  bluetoothSerial.begin(9600);
  Serial.println("HC-06 Bluetooth Communication Initialized on Pins 10 (RX) and 11 (TX)");
  Keyboard.releaseAll();
...
void loop() {
  ...
  if(frame==0){
   // PushIntegerToUART("58008");
    Serial.println("Hello Keyboard.H");
  }
  ...
  if(Serial.available())
  {
    ...
    if(useDebugPrint) 
      Serial.println("Serial:"+String(read));
    int32_t  integer= read.toInt();
    integerToActions(integer);
  }
