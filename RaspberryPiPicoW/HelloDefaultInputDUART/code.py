...
USE_WIFI = True
UDP_IP_TARGET = "192.168.1.253"
UDP_PORT_TEXT = 7031
UDP_PORT_INTEGER = 7073
UDP_INDEX_PLAYER=1
...
def try_to_send_UTF8(text):
    if not USE_WIFI:
        return
    pool = socketpool.SocketPool(wifi.radio)
    udp_socket = pool.socket(pool.AF_INET, pool.SOCK_DGRAM)
    try:
        udp_socket.sendto(bytes(text, "utf-8"), (UDP_IP_TARGET, UDP_PORT_TEXT))
        print(f"Sent message: {text} to {UDP_IP_TARGET}:{UDP_PORT_TEXT}")
    except:
        print(f"Failed to send Integer: {text} to {UDP_IP_TARGET}:{UDP_PORT_INTEGER}")
    
    finally:
        udp_socket.close()
        
def try_to_send_integer(integer:int):
    if not USE_WIFI:
        return
    ...
    udp_socket = pool.socket(pool.AF_INET, pool.SOCK_DGRAM)
    try:
        udp_socket.sendto(byte_integer_value, (UDP_IP_TARGET, UDP_PORT_INTEGER))
        print(f"Sent Integer: {integer} to {UDP_IP_TARGET}:{UDP_PORT_INTEGER}")
    except:
        print(f"Failed to send Integer: {integer} to {UDP_IP_TARGET}:{UDP_PORT_INTEGER}")
    
    finally:
        udp_socket.close()
        
def try_to_send_index_integer_default(integer:int):
    try_to_send_index_integer(UDP_INDEX_PLAYER,integer)
    
def try_to_send_index_integer_all(integer:int):
    try_to_send_index_integer(0,integer)
        
def try_to_send_index_integer(index:int,integer:int):
    if not USE_WIFI:
        return
    ...
    try:
        udp_socket.sendto(byte_integer_value, (UDP_IP_TARGET, UDP_PORT_INTEGER))
        print(f"Sent II: {index} {integer} to {UDP_IP_TARGET}:{UDP_PORT_INTEGER}")
    except:
        print(f"Failed to send II: {index} {integer} to {UDP_IP_TARGET}:{UDP_PORT_INTEGER}")
    
    finally:
        udp_socket.close()


bool_websocket_experiment = True
if bool_websocket_experiment:
    try_to_connect_wifi()
    time.sleep(1)
    try_to_send_UTF8("Hello World")
    try_to_send_integer(42)
    try_to_send_index_integer(4,42)
...
def press_button_index_1_32(index:int):
    gamepad.press_buttons(index)
    midi.send(NoteOn(60+index, 127))
    
def release_button_index_1_32(index:int):
    if index<=0 or index>32: 
        print("Error index must be between 1 and 32: ", str(index))
...
def press_or_release_button_index_1_16(index:int, bool_press:bool):
    if bool_press:
        press_button_index_1_32(index)
    else:
        release_button_index_1_32(index)
...
def uartToAction(c0, c1):
    ...
    if c1 == '1':
        ...
        if bool_is_a_to_z:
            int_index =(byte_value-96)
            midi.send(NoteOff(60+int_index, 120))
            if int_index<32:
                gamepad.press_buttons(int_index+1)
        if bool_is_A_to_Z:
            int_index =(byte_value-64)
            midi.send(NoteOn(60+int_index, 120))
            if int_index<32:
                gamepad.release_buttons(int_index+1)
            
    if c1== '2':
        if c0=='L':
            keyboard.press(Keycode.LEFT_ARROW)
            gamepad.set_joystick_right_x_percent(-1)
            try_to_send_index_integer_default(1037)
        elif c0=='D':
            keyboard.press(Keycode.DOWN_ARROW)
            gamepad.set_joystick_right_y_percent(-1)
            try_to_send_index_integer_default(1040)
        elif c0=='R':
            keyboard.press(Keycode.RIGHT_ARROW)
            gamepad.set_joystick_right_x_percent(1)
            try_to_send_index_integer_default(1039)
        elif c0=='U':
            keyboard.press(Keycode.UP_ARROW)
            gamepad.set_joystick_right_y_percent(1)
            try_to_send_index_integer_default(1038)
        elif c0=='O':
            ...
            try_to_send_index_integer_default(2037)
        elif c0=='d':
            keyboard.release(Keycode.DOWN_ARROW)
            try_to_send_index_integer_default(2040)
        elif c0=='r':
            keyboard.release(Keycode.RIGHT_ARROW)
            try_to_send_index_integer_default(2039)
        elif c0=='u':
            keyboard.release(Keycode.UP_ARROW)
            try_to_send_index_integer_default(2038)
        elif c0=='e':
    ...
    while True:
        ...
        if bool_pin_2_state != pin_2_HW763_red.value:
            ...
            if pin_2_HW763_red.value:
                print("Pin 2 on")
                try_to_send_index_integer_all(1032)
            else:
                print("Pin 2 off")
                try_to_send_index_integer_all(2032)
        ...
        if bool_pin_14_state != pin_14_poximity_button.value:
            ...
            if pin_14_poximity_button.value:
                print("Pin 14 on")
                try_to_send_index_integer_all(1009)
            else :
                print("Pin 14 off")
                try_to_send_index_integer_all(2009)
        ...
        if bool_is_joystick_left != bool_is_joystick_left_previous:
            ...
            if bool_is_joystick_left:
                ...
                try_to_send_index_integer_all(1037)
            else:
                print("Joystick Left End")
                #turn_middle(pin_16_SG90_write)
                try_to_send_index_integer_all(2037)
        ...
        if bool_is_joystick_right != bool_is_joystick_right_previous:
            ...
            if bool_is_joystick_right:
                ...
                try_to_send_index_integer_all(1039)
                #turn_right_by_middle(pin_16_SG90_write)
            else:
                print("Joystick Right End")
                #turn_middle(pin_16_SG90_write)
                try_to_send_index_integer_all(2039)
            bool_print_joystick=True
        ...
        if bool_is_joystick_up != bool_is_joystick_up_previous:
            ...
            if bool_is_joystick_up:
                ...
                try_to_send_index_integer_all(1038)
            else:
                print("Joystick Up End")
                try_to_send_index_integer_all(2038)
            bool_print_joystick=True
        if bool_is_joystick_down != bool_is_joystick_down_previous:
            ...
            if bool_is_joystick_down:
                ...
                try_to_send_index_integer_all(1040)
            else:
                print("Joystick Down End")
                try_to_send_index_integer_all(2040)
            bool_print_joystick=True
